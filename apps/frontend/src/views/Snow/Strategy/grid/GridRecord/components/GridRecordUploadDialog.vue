<template>
  <Dialog v-model="dialogShow" :title="dialogTitle" center>
    <Form ref="formRef" :is-col="false" :schema="uploadFormSchema">
      <template #file>
        ...
        >
          ...
          <template #tip>
            <div class="el-upload__tip">请上传交易XLSX格式的记录文件，包含（交易时间	交易价格	交易份额	交易费用）中文列 </div>
          </template>
        </el-upload>
      </template>
    </Form>
...
const uploadInfo = reactive({
  // 上传的地址
  url: import.meta.env.VITE_API_BASEPATH + '/record_file',
  data: {
...
watch(
  () => props.gridTypeId,
  (newGridTypeId) => {
    uploadInfo.data.gridTypeId = newGridTypeId
  }
)
...
const confirmDialog = () => {
  ...
  const separator = _.get(unref(formRef)?.formModel, 'separator', null)
  const encoding = _.get(unref(formRef)?.formModel, 'encoding', null)
  uploadInfo.data.separator = separator
  uploadInfo.data.encoding = encoding
