...
- lite 下优先验证：系统设置、资产列表、记录列表、指数列表、grid / grid-type 分析结果、通知列表这类读链路
- lite 下显式不支持：scheduler 页面、系统 token / Redis 相关配置页
...
- 包管理器选择？
  - 这份工作区只走 `pnpm`。
- 为什么 lite 默认不开放 scheduler 和系统 token？
  - 当前 lite 后端默认关闭 `ENABLE_SCHEDULER`、`ENABLE_REDIS`，这些页面会显式降级，不再假装可用。
