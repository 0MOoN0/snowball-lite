...
- lite 主线不是只换数据库，而是把一批重运行时组件改成默认关闭或明确不支持
...
| APScheduler 调度器 | 默认启用 | 默认关闭 | 需要时可临时打开，但不是默认前提 |
| APScheduler 持久化 JobStore | 默认启用 | 默认关闭 | lite 不保证持久化 scheduler 能力 |
