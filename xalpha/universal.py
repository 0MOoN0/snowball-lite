...
from xalpha.sqlio import CACHE_BACKEND_MISS_ERRORS, read_sql_table_compat
...
# @lru_cache_time(ttl=300) 自动获取token已经失效，项目在初始化时会自动设置token，因此不会有频繁获取token的问题，所以不需要再使用lru_cache_time
        ...
        def wrapper(*args, **kws):
            ...
            if not backend:
                ...
                if refresh:
                    ...
                    try:
                        if backend == "csv":
                            ...
                            if key in getattr(thismodule, "cached_dict"):
                                ...
                                df0 = read_sql_table_compat(key, path)
                        elif backend == "memory":
                            df0 = getattr(thismodule, "cached_dict")[key]
                        else:
                    ...
                    except CACHE_BACKEND_MISS_ERRORS as e:
                        if fetchonly:
                            logger.error(
...
def fetch_backend(key):
    ...
    except CACHE_BACKEND_MISS_ERRORS:
        return None
