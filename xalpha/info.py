...
from xalpha.sqlio import (
    CACHE_BACKEND_MISS_ERRORS,
    normalize_sql_date_column,
    read_sql_table_compat,
)
...
class fundinfo(basicinfo):
    ...
    def _fetch_sql(self, path):
        ...
        """
        try:
            content = normalize_sql_date_column(
                read_sql_table_compat("xa" + self.code, path)
            )
            pricetable = content.iloc[1:]
